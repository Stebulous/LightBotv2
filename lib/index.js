...
function GetPermissionLevel(user) {
  ...
  if (guildmemberobject == true) {
    if (user.guild.owner == user) {
      ...
      user.hasRole('383701695026429952')) {
      presumed_rank = 5;
    }
  }
...
bot.on('message', msg => {
  ...
  if (commands[command]) {
    ...
    msg.channel.send(`:x: The command \`${command}\` could not be found!`).then(m => {
      m.delete(10000).then(mesg.delete());
    })
  }
});
...
function load() {
  ...
  fs.readdirSync('./lib/commands/').forEach(file => {
    ...
    var command = require('./lib/commands/' + file);
  ...
  bot.user.setGame(`LightBot.tk | ${config.prefix}help`);
}
